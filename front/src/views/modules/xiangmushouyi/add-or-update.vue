  ...
  <div class="addEdit-block">
    ...
    >
      <el-row>
          ...
          <el-form-item class="input" label="签单金额" prop="qiandanjine">
              <el-input v-model="ruleForm.qiandanjine" 
                placeholder="签单金额" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="分配类型" prop="fenpeileixing">
          <el-select v-model="ruleForm.fenpeileixing" placeholder="请选择分配类型">
            <el-option
                v-for="(item,index) in fenpeileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="分配类型" prop="fenpeileixing">
	      <el-input v-model="ruleForm.fenpeileixing"
                placeholder="分配类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="分配金额" prop="fenpeijine">
          <el-input v-model="ruleForm.fenpeijine" 
              placeholder="分配金额" clearable  :readonly="ro.fenpeijine"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="分配金额" prop="fenpeijine">
              <el-input v-model="ruleForm.fenpeijine" 
                placeholder="分配金额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.qiandanjine = true;
            continue;
	            	            ...
	            	            if(o=='fenpeileixing'){
            this.ruleForm.fenpeileixing = obj[o];
	    this.ro.fenpeileixing = true;
            continue;
          }
	            	            if(o=='fenpeijine'){
            this.ruleForm.fenpeijine = obj[o];
	    this.ro.fenpeijine = true;
            continue;
          }
	            	            if(o=='riqi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                                                ...
                                                                                                this.fenpeileixingOptions = "集团,售前,售后,部门".split(',')
                                                                },
                                                        // 多级联动参数
                                                                        info(id) {
      this.$http({
        url: `xiangmushouyi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `xiangmushouyi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiangmushouyiCrossAddOrUpdateFlag = false;
                  this.parent.search();
